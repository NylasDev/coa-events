...
class EventNotifier {
     ...
     */
    async initialize() {
        console.log('🔔 Checking event notification system configuration...');
        
        // Check if webhook URL is configured
        if (!process.env.DISCORD_WEBHOOK_URL) {
            console.warn('⚠️ Discord webhook URL not configured. Discord notifications will be disabled.');
            return false;
        }
        
        // Check if bot token is configured
        if (!process.env.DISCORD_BOT_TOKEN) {
            console.warn('⚠️ Discord bot token not configured. Some Discord features will be limited.');
        ...
        try {
            ...
            console.log('✅ Event notification system initialized successfully');
            return true;
        } catch (error) {
            console.error('⚠️ Error initializing notification system:', error.message);
            return false;
        }
    }
