...
class DiscordUtils {
    ...
    // Send message directly to a Discord channel (requires bot token and permissions)
    async sendChannelMessage(channelId, options) {
        ...
        try {
            ...
            // Handle both simple content and embed options
            if (typeof options === 'string') {
                messageData.content = options;
            } else {
            ...
            const response = await this.client.post(`/channels/${channelId}/messages`, messageData);
            return response.data;
        } catch (error) {
    ...
    async notifyEventUpdate(event, signups = [], timeUntil = null) {
        ...
        // Try to send directly to channel if channel ID is configured and bot token is available
        const channelId = process.env.DISCORD_EVENTS_CHANNEL_ID;
        if (channelId && this.botToken) {
            try {
                console.log(`📤 Sending event notification directly to channel ${channelId}`);
                return await this.sendChannelMessage(channelId, messageData);
            } catch (error) {
                console.warn(`⚠️ Failed to send direct message to channel ${channelId}, falling back to webhook:`, error.message);
                // Fall back to webhook if direct messaging fails
            }
        }
        
        // Fallback to webhook
        console.log('📤 Sending event notification via webhook');
        return this.sendWebhookMessage(messageData);
    }
